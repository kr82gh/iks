...
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete catalogsources.operators.coreos.com >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
...
if [ "$question13answer" !=  "${question12answer#[Yy]}" ]; then 
 echo "Deleting CRD clusterserviceversions.operators.coreos.com "
         echo "Running ${KUBECTL} delete clusterserviceversions.operators.coreos.com" >> ${DELETELOG} 2>&1
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete clusterserviceversions.operators.coreos.com  >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
     echo "ERROR: ${KUBECTL} delete clusterserviceversions.operators.coreos.com  !"
      else
     echo "INFO: Successfully deleted"
    fi 
  else 
...
if [ "$question14answer" !=  "${question14answer#[Yy]}" ]; then 
 echo "Deleting CRD installplans.operators.coreos.com "
         echo "Running ${KUBECTL} delete installplans.operators.coreos.com " >> ${DELETELOG} 2>&1
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete installplans.operators.coreos.com >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
     echo "ERROR: ${KUBECTL} delete installplans.operators.coreos.com  !"
      else
     echo "INFO: Successfully deleted"
    fi 
  else 
...
if [ "$question15answer" !=  "${question15answer#[Yy]}" ]; then 
 echo "Deleting CRD operatorgroups.operators.coreos.com  "
         echo "Running ${KUBECTL} delete operatorgroups.operators.coreos.com  " >> ${DELETELOG} 2>&1
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete operatorgroups.operators.coreos.com >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
     echo "ERROR: ${KUBECTL} delete operatorgroups.operators.coreos.com  !"
      else
     echo "INFO: Successfully deleted"
    fi 
  else 
...
if [ "$question16answer" !=  "${question16answer#[Yy]}" ]; then 
 echo "Deleting CRD operators.operators.coreos.com  "
         echo "Running ${KUBECTL} delete operators.operators.coreos.com   " >> ${DELETELOG} 2>&1
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete operators.operators.coreos.com  >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
     echo "ERROR: ${KUBECTL} delete operators.operators.coreos.com  !"
      else
     echo "INFO: Successfully deleted"
    fi 
  else 
...
if [ "$question17answer" !=  "${question17answer#[Yy]}" ]; then 
 echo "Deleting CRD subscriptions.operators.coreos.com"
         echo "Running ${KUBECTL} delete subscriptions.operators.coreos.com" >> ${DELETELOG} 2>&1
# Delete object kubectl command below. Output and errors are directed to log file  
    ${KUBECTL} delete subscriptions.operators.coreos.com  >> ${DELETELOG} 2>&1
    if [ $? != '0' ]; then
     echo "ERROR: ${KUBECTL} delete subscriptions.operators.coreos.com !"
      else
     echo "INFO: Successfully deleted"
    fi 
  else 
